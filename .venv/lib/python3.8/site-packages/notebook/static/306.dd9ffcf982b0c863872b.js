...
(self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] = self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] || []).push([
  [306],
  {
    /***/ 50306: /***/ (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
      ...
      class SpecializeExpression extends Expression {
        ...
        eq(other) {
          return (
            this.type == other.type && Prop.eqProps(this.props, other.props) && exprEq(this.token, other.token) && exprEq(this.content, other.content)
          );
        }
        walk(f) {
          ...
          return f(
            token == this.token && content == this.content ? this : new SpecializeExpression(this.start, this.type, this.props, token, content)
          );
        }
      }
      ...
      class Prop extends Node {
        ...
        toString() {
          ...
          if (this.value.length) {
            result += "=";
            for (let { name, value } of this.value) result += name ? `{${name}}` : /[^\w-]/.test(value) ? JSON.stringify(value) : value;
          }
          return result;
        }
        static eqProps(a, b) {
...
*/
      ...
      class Term {
        ...
        set preserve(value) {
          this.flags = value ? this.flags | 8 /* TermFlag.Preserve */ : this.flags & ~8 /* TermFlag.Preserve */;
        }
        get preserve() {
          return (this.flags & 8) /* TermFlag.Preserve */ > 0;
        }
        set inline(value) {
          this.flags = value ? this.flags | 32 /* TermFlag.Inline */ : this.flags & ~32 /* TermFlag.Inline */;
        }
        get inline() {
      ...
      class Edge {
        ...
        toString() {
          return `-> ${this.target.id}[label=${JSON.stringify(
            this.from < 0 ? "ε" : charFor(this.from) + (this.to > this.from + 1 ? "-" + charFor(this.to - 1) : "")
          )}]`;
        }
      }
      function charFor(n) {
      ...
      function isEquivalent(a, b, partition) {
        ...
        for (let i = 0; i < a.edges.length; i++) {
          let eA = a.edges[i],
            eB = b.edges[i];
          if (eA.from != eB.from || eA.to != eB.to || partition[eA.target.id] != partition[eB.target.id]) return false;
        }
        return true;
      }
      ...
      let State$1 = class State {
        ...
        findConflicts(occurTogether) {
          ...
          function add(a, b, soft, aEdges, bEdges) {
            ...
            if (!found) conflicts.push(new Conflict$1(a, b, soft, exampleFromEdges(aEdges), bEdges && exampleFromEdges(bEdges)));
          ...
          this.reachable((state, edges) => {
            ...
            for (let i = 0; i < state.accepting.length; i++)
              for (let j = i + 1; j < state.accepting.length; j++) add(state.accepting[i], state.accepting[j], 0, edges);
        ...
        toString() {
          let out = "digraph {\n";
          this.reachable((state) => {
            if (state.accepting.length) out += `  ${state.id} [label=${JSON.stringify(state.accepting.join())}];\n`;
        ...
        toArray(groupMasks, precedence) {
          ...
          if (data.length > Math.pow(2, 16)) throw new GenError("Tokenizer tables too big to represent with 16-bit offsets.");
      ...
      function mergeEdges(edges) {
        ...
        if (eof.length) {
          ...
          if (found.length) result.push(new MergedEdge(65535 /* Seq.End */, 65535 /* Seq.End */, found));
        }
        return result;
      }
      ...
      function parseExprInner(input) {
        let start = input.start;
        if (input.eat("(")) {
          if (input.eat(")")) return new SequenceExpression(start, none$2, [none$2, none$2]);
          let expr = parseExprChoice(input);
          input.expect(")");
          return expr;
          ...
          input.next();
          if (value.length == 0) return new SequenceExpression(start, none$2, [none$2, none$2]);
      ...
      function addRange(input, ranges, from, to) {
        if (!ranges.every(([a, b]) => b <= from || a >= to)) input.raise("Overlapping character range", input.start);
        ranges.push([from, to]);
      }
      function parseExprSuffix(input) {
        ...
        for (;;) {
          let kind = input.type;
          if (input.eat("*") || input.eat("?") || input.eat("+")) expr = new RepeatExpression(start, expr, kind);
          else return expr;
        }
      }
      ...
      function parseTokenConflict(input) {
        ...
        if (!(a instanceof LiteralExpression || a instanceof NameExpression)) input.raise(`Invalid expression in token conflict`, a.start);
        input.eat(",");
        let b = parseExprInner(input);
        if (!(b instanceof LiteralExpression || b instanceof NameExpression)) input.raise(`Invalid expression in token conflict`, b.start);
        input.expect("}");
      ...
      class Reduce {
        ...
        cmp(other) {
          return other instanceof Shift
            ? 1
            : this.term.id - other.term.id || this.rule.name.id - other.rule.name.id || this.rule.parts.length - other.rule.parts.length;
        }
        matches(other, mapping) {
      ...
      class State {
        ...
        toString() {
          let actions =
            this.actions.map((t) => t.term + "=" + t).join(",") + (this.goto.length ? " | " + this.goto.map((g) => g.term + "=" + g).join(",") : "");
          return (
            this.id +
            ": " +
        ...
        addAction(value, positions, context) {
          ...
          if (conflict) {
            ...
            let error;
            if (conflict instanceof Shift) error = `shift/reduce conflict between\n  ${conflictPos}\nand\n  ${positions[0].rule}`;
            else error = `reduce/reduce conflict between\n  ${conflictPos.rule}\nand\n  ${positions[0].rule}`;
            error += `\nWith input:\n  ${positions[0].trail(70)} · ${value.term} …`;
            if (conflict instanceof Shift) error += findConflictShiftSource(positions[0], conflict.term, context.first);
        ...
        actionsByTerm() {
          ...
          if (!result) {
            ...
            for (let action of this.actions) (result[action.term.id] || (result[action.term.id] = [])).push(action);
          }
          return result;
        }
        finish() {
          if (this.actions.length) {
            ...
            if (first instanceof Reduce) {
              let { rule } = first;
              if (this.actions.every((a) => a instanceof Reduce && a.rule.sameReduce(rule))) this.defaultReduce = rule;
            }
          }
      ...
      function closure(set, first) {
        ...
        function addFor(name, ahead, ambigAhead, skipAhead, via) {
          for (let rule of name.rules) {
            ...
            if (!add) {
              ...
              add = existing
                ? new Pos(rule, 0, existing.ahead.slice(), existing.ambigAhead, existing.skipAhead, existing.via)
                : new Pos(rule, 0, [], none$1, skipAhead, via);
              added.push(add);
            }
            if (add.skipAhead != skipAhead) throw new GenError("Inconsistent skip sets after " + via.trail());
        ...
        while (redo.length) {
          ...
          addFor(
            ...
            union(add.rule.conflicts[1].ambigGroups, add.rule.parts.length == 1 ? add.ambigAhead : none$1),
      ...
      function findConflictShiftSource(conflictPos, termAfter, first) {
        ...
        function explore(pos, i, hasMatch) {
          ...
          for (let next; (next = pos.next); ) {
            ...
            if (after && (after.terminal ? after == termAfter : first[after.name].includes(termAfter))) match = pos.advance();
      ...
      function buildFullAutomaton(terms, startTerms, first) {
        ...
        function getState(core, top) {
          ...
          if (!found) {
            ...
            forHash.push(found);
            states.push(found);
            if (timing && states.length % 500 == 0) console.log(`${states.length} states after ${((Date.now() - t0) / 1000).toFixed(2)}s`);
          }
        ...
        for (const startTerm of startTerms) {
          ...
          getState(
            startTerm.rules.map((rule) => new Pos(rule, 0, [terms.eof], none$1, startSkip, null).finish()),
            startTerm
          );
        }
        ...
        for (let filled = 0; filled < states.length; filled++) {
          ...
          if (replaced)
            for (let i = 0; i < state.goto.length; i++) {
              ...
              if (!start.some((term) => state.actions.some((a) => a.term == term && a instanceof Shift))) state.goto.splice(i--, 1);
            }
        }
      ...
      function canMerge(a, b, mapping) {
        ...
        for (let action of a.actions) {
          ...
          if (setB && setB.some((other) => !other.matches(action, mapping))) {
            ...
            if (setA.length != setB.length || setA.some((a1) => !setB.some((a2) => a1.matches(a2, mapping)))) return false;
          }
        }
        return true;
      }
      ...
      function encode(value, max = 0xffff) {
        ...
        if (value == 65535 /* Encode.BigVal */) return String.fromCharCode(126 /* Encode.BigValCode */);
        let result = "";
      ...
      class Builder {
        constructor(text, options) {
          ...
          time("Build rules", () => {
            ...
            for (let scoped of this.ast.scopedSkip) {
              let skip = noSkip,
                found = this.ast.scopedSkip.findIndex((sc, i) => i < scopedSkip.length && exprEq(sc.expr, scoped.expr));
            ...
            for (let i = 0; i < this.ast.scopedSkip.length; i++) {
              ...
              if (!this.skipRules.includes(skip)) {
                ...
                if (skip != noSkip) this.defineRule(skip, this.normalizeExpr(this.ast.scopedSkip[i].expr));
              }
            }
            ...
            for (let { skip, rule } of this.astRules) {
              if (this.ruleNames[rule.id.name] && isExported(rule) && !rule.params.length) {
                ...
                if (rule.expr instanceof SequenceExpression && rule.expr.exprs.length == 0) this.used(rule.id.name);
              }
            }
          });
        ...
        unique(id) {
          if (id.name in this.ruleNames) this.raise(`Duplicate definition of rule '${id.name}'`, id.start);
        ...
        newName(base, nodeName = null, props = {}) {
          for (let i = nodeName ? 0 : 1; ; i++) {
            ...
            if (!this.terms.names[name]) return this.terms.makeNonTerminal(name, nodeName === true ? null : nodeName, props);
          }
        }
        prepareParser() {
          let rules = time("Simplify rules", () => simplifyRules(this.rules, [...this.skipRules, ...this.terms.tops]));
          ...
          let skipInfo = this.skipRules.map((name, id) => {
            ...
            for (let rule of name.rules) {
              ...
              for (let t of start.terminal ? [start] : first[start.name] || []) if (t && !startTokens.includes(t)) startTokens.push(t);
              if (start.terminal && rule.parts.length == 1 && !rules.some((r) => r != rule && r.parts[0] == start)) skip.push(start);
          ...
          for (let name in this.specialized) specialized.push({ token: this.terms.names[name], table: buildSpecializeTable(this.specialized[name]) });
          ...
          let skipData = skipInfo.map((info) => {
            let actions = [];
            for (let term of info.skip) actions.push(term.id, 0, 262144 /* Action.StayFlag */ >> 16);
            if (info.rule) {
              let state = table.find((s) => s.startRule == info.rule);
              for (let action of state.actions) actions.push(action.term.id, state.id, 131072 /* Action.GotoFlag */ >> 16);
            }
          ...
          for (let term of this.terms.tops) topRules[term.nodeName] = [table.find((state) => state.startRule == term).id, term.id];
          ...
          let specialized = rawSpecialized.map((v) => {
            if (v instanceof ExternalSpecializer) {
              ...
              return {
                term: v.term.id,
                get: (value, stack) =>
                  (ext(value, stack) << 1) | (v.ast.type == "extend" ? 1 /* Specialize.Extend */ : 0) /* Specialize.Specialize */,
                external: ext,
        ...
        getParserFile() {
          ...
          function specializationTableString(table) {
            return (
              "{__proto__:null," +
              Object.keys(table)
                .map((key) => `${/^(\d+|[a-zA-Z_]\w*)$/.test(key) ? key : JSON.stringify(key)}:${table[key]}`)
                .join(", ") +
              "}"
            );
          }
          ...
          let specialized = rawSpecialized.map((v) => {
            if (v instanceof ExternalSpecializer) {
              ...
              return `{term: ${v.term.id}, get: (value${ts}, stack${ts}) => (${name}(value, stack) << 1)${
                v.ast.type == "extend" ? ` | ${1 /* Specialize.Extend */}` : ""
  ...
  termNames: ${JSON.stringify(termNames)}`
      : ""
          ...
          for (let id = 0; id < this.dialects.length; id++) terms.push(`Dialect_${this.dialects[id]}${mod == "cjs" ? ":" : " ="} ${id}`);
          return {
        ...
        substituteArgs(expr, args, params) {
          ...
          return expr.walk((expr) => {
            let found;
            if (expr instanceof NameExpression && (found = params.findIndex((p) => p.name == expr.id.name)) > -1) {
              let arg = args[found];
              if (expr.args.length) {
                if (arg instanceof NameExpression && !arg.args.length) return new NameExpression(expr.start, arg.id, expr.args);
              ...
              return props == r.props ? expr : new InlineRuleExpression(expr.start, new RuleDeclaration(r.start, r.id, props, r.params, r.expr));
            } else if (expr instanceof SpecializeExpression) {
              let props = this.substituteArgsInProps(expr.props, args, params);
              return props == expr.props ? expr : new SpecializeExpression(expr.start, expr.type, props, expr.token, expr.content);
            }
            return expr;
          });
        }
        substituteArgsInProps(props, args, params) {
          let substituteInValue = (value) => {
            ...
            for (let i = 0; i < value.length; i++) {
              ...
              let expr = args[found];
              if (expr instanceof NameExpression && !expr.args.length) result[i] = new PropPart(part.start, expr.id.name, null);
              else if (expr instanceof LiteralExpression) result[i] = new PropPart(part.start, expr.value, null);
        ...
        defineRule(name, choices) {
          ...
          for (let choice of choices) this.rules.push(new Rule(name, choice.terms, choice.ensureConflicts(), skip));
        }
        resolve(expr) {
          ...
          if (known.rule.params.length != expr.args.length) this.raise(`Wrong number or arguments for '${expr.id.name}'`, expr.start);
        ...
        nodeInfo(
          ...
          for (let prop of props) {
            if (!prop.at) {
              ...
              if (allow.indexOf("d") < 0) this.raise("Can't specify a dialect on non-token rules", props[0].start);
              if (prop.value.length != 1 && !prop.value[0].value) this.raise("The '@dialect' rule prop must hold a plain string value");
              ...
              if (allow.indexOf("p") < 0) this.raise("Dynamic precedence can only be specified on nonterminals");
          ...
          if (hasProps(result) && !name) this.raise(`Node has properties but no name`, props.length ? props[0].start : expr.start);
        ...
        resolveSpecialization(expr) {
          ...
          let values;
          if (expr.content instanceof LiteralExpression) values = [expr.content.value];
          ...
          else return this.raise(`The second argument to '${expr.type}' must be a literal or choice of literals`, expr.content.start);
          ...
          for (let value of values) {
            ...
            if (known == null) {
              ...
              if (known.name != name) this.raise(`Conflicting names for specialization ${JSON.stringify(value)} of ${term.name}`, expr.start);
        ...
        defineGroup(rule, group, ast) {
          ...
          for (let name of getNamed(rule))
            name.props["group"] = (((_a = name.props["group"]) === null || _a === void 0 ? void 0 : _a.split(" ")) || [])
              .concat(group)
              .sort()
              .join(" ");
        }
        ...
        storeActions(actions, skipReduce, shared) {
          ...
          for (let action of actions) {
            ...
            if (action instanceof Shift) {
              ...
              if (code != skipReduce) data.push(action.term.id, code & 65535 /* Action.ValueMask */, code >> 16);
            }
          }
          data.push(65535 /* Seq.End */);
          if (skipReduce > -1) data.push(2 /* Seq.Other */, skipReduce & 65535 /* Action.ValueMask */, skipReduce >> 16);
          else if (shared) data.push(1 /* Seq.Next */, shared.addr & 0xffff, shared.addr >> 16);
          ...
          if (defaultReduce == 0) {
            if (isSkip)
              for (const action of state.actions)
                if (action instanceof Reduce && action.term.eof) skipReduce = reduceAction(action.rule, this.skipInfo);
            if (skipReduce < 0) shared = this.findSharedActions(state);
          }
          if (state.set.some((p) => p.rule.name.top && p.pos == p.rule.parts.length)) flags |= 2 /* StateFlag.Accepting */;
          let external = [];
      ...
      class TokenSet {
        ...
        buildRule(rule, expr, from, to, args = none) {
          let name = expr.id.name;
          if (rule.params.length != expr.args.length) this.b.raise(`Incorrect number of arguments for token '${name}'`, expr.start);
        ...
        build(expr, from, to, args) {
          if (expr instanceof NameExpression) {
            ...
            for (let [a, b] of expr.inverted ? invertRanges(expr.ranges) : expr.ranges) rangeEdges(from, to, a, b);
          } else if (expr instanceof LiteralExpression) {
            ...
            for (let group of this.ast.precedences) {
              ...
              for (let item of group.items) {
                ...
                if (item instanceof NameExpression) {
                  for (let built of this.built) if (item.args.length ? built.matches(item) : built.id == item.id.name) level.push(built.term);
                } else {
      ...
      class MainTokenSet extends TokenSet {
        ...
        buildTokenGroups(states, skipInfo, startID) {
          ...
          for (let { a, b } of this.explicitConflicts) {
            if (!allConflicts.some((c) => c.a == a && c.b == b)) allConflicts.push(new Conflict$1(a, b, 0, "", ""));
          }
          ...
          for (let state of states) {
            ...
            for (let term of skip) if (state.actions.some((a) => a.term == term)) this.b.raise(`Use of token ${term.name} conflicts with skip rule`);
            let stateTerms = [];
          ...
          if (groups.length + startID > 16) this.b.raise(`Too many different token groups (${groups.length}) to represent them as a 16-bit bitfield`);
        ...
        buildLocalGroup(states, skipInfo, id) {
          ...
          for (let state of states) {
            ...
            for (let { term } of state.actions) {
              let orig = this.b.tokenOrigins[term.name];
              if ((orig === null || orig === void 0 ? void 0 : orig.group) == this) usesThis = term;
              else usesOther = term;
            }
            if (usesThis) {
              if (usesOther)
      ...
      class ExternalSpecializer {
        ...
        finish() {
          ...
          for (let name in this.tokens) this.b.tokenOrigins[this.tokens[name].name] = { spec: this.term, external: this };
        }
        getToken(expr) {
          return findExtToken(this.b, this.tokens, expr);
        }
