...
(self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] = self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] || []).push([
  [1088],
  {
    /***/ 31088: /***/ (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
        ...
        if (sol)
          if (c == "/") return (state.tokenize = tokenLineComment)(stream, state);
            ...
            if (stream.eol() || /\s/.test(stream.peek()))
              return (
                stream.skipToEnd(),
                /^\\\s*$/.test(stream.current()) ? (state.tokenize = tokenCommentToEOF)(stream) : (state.tokenize = tokenBase),
                "comment"
              );
      ...
      function tokenLineComment(stream, state) {
        return (
          stream.skipToEnd(),
          /\/\s*$/.test(stream.current()) ? (state.tokenize = tokenBlockComment)(stream, state) : (state.tokenize = tokenBase),
          "comment"
        );
      }
