...
(self["webpackChunk_jupyterlab_application_top"] = self["webpackChunk_jupyterlab_application_top"] || []).push([
  [4364],
  {
    54364: (e, t, n) => {
      ...
      const a = /[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/;
      ...
      function y(e, t, n, s) {
        const r = t.href;
        const i = t.title ? u(t.title) : null;
      ...
      class z {
        ...
        code(e) {
          ...
          if (t) {
            ...
            return { type: "code", raw: t[0], codeBlockStyle: "indented", text: !this.options.pedantic ? w(e, "\n") : e };
          }
        }
        fences(e) {
        ...
        blockquote(e) {
          ...
          if (t) {
            ...
            return { type: "blockquote", raw: t[0], tokens: s, text: e };
          }
        }
        list(e) {
          ...
          if (t) {
            ...
            while (e) {
              ...
              if (!n) {
                ...
                const r = new RegExp(`^ {0,${Math.min(3, h - 1)}}(?:\`\`\`|~~~)`);
                const i = new RegExp(`^ {0,${Math.min(3, h - 1)}}#`);
                while (e) {
                  const a = e.split("\n", 1)[0];
                  c = a;
                  if (this.options.pedantic) {
              ...
              r.items.push({ type: "list_item", raw: l, task: !!u, checked: f, loose: false, text: o, tokens: [] });
              r.raw += l;
            }
        ...
        def(e) {
          ...
          if (t) {
            ...
            const n = t[2] ? t[2].replace(/^<(.*)>$/, "$1").replace(this.rules.inline._escapes, "$1") : "";
            const s = t[3] ? t[3].substring(1, t[3].length - 1).replace(this.rules.inline._escapes, "$1") : t[3];
        ...
        table(e) {
          ...
          if (t) {
            ...
            const e = {
              type: "table",
              raw: t[0],
              header: m(t[1]).map((e) => ({ text: e, tokens: [] })),
              align: t[2].replace(/^\||\| *$/g, "").split("|"),
              rows: t[3] && t[3].trim() ? t[3].replace(/\n[ \t]*$/, "").split("\n") : [],
            };
            if (e.header.length === e.align.length) {
              let t = e.align.length;
              let n, s, r, i;
              for (n = 0; n < t; n++) {
                const t = e.align[n];
                if (t) {
              ...
              t = e.rows.length;
              for (n = 0; n < t; n++) {
                e.rows[n] = m(e.rows[n], e.header.length).map((e) => ({ text: e, tokens: [] }));
              }
              t = e.header.length;
              for (s = 0; s < t; s++) {
        ...
        tag(e) {
          ...
          if (t) {
            ...
            if (!this.lexer.state.inRawBlock && /^<(pre|code|kbd|script)(\s|>)/i.test(t[0])) {
              this.lexer.state.inRawBlock = true;
            } else if (this.lexer.state.inRawBlock && /^<\/(pre|code|kbd|script)(\s|>)/i.test(t[0])) {
        ...
        link(e) {
          ...
          if (t) {
            ...
            return y(
              t,
              { href: n ? n.replace(this.rules.inline._escapes, "$1") : n, title: s ? s.replace(this.rules.inline._escapes, "$1") : s },
              t[0],
              this.lexer
            );
          }
        ...
        emStrong(e, t, n = "") {
          ...
          if (!r || !n || this.rules.inline.punctuation.exec(n)) {
            ...
            a.lastIndex = 0;
            t = t.slice(-1 * e.length + s[0].length - 1);
        ...
        inlineText(e) {
          ...
          if (t) {
            ...
            return { type: "text", raw: t[0], text: e };
          }
        }
      }
      const R = {
      ...
      R._label = /(?!\s*\])(?:\\.|[^\[\]\\])+/;
      R._title = /(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/;
      R.def = d(R.def).replace("label", R._label).replace("title", R._title).getRegex();
      R.bullet = /(?:[*+-]|\d{1,9}[.)])/;
      R.listItemStart = d(/^( *)(bull) */)
      ...
      R.html = d(R.html, "i")
        ...
        .getRegex();
      R.lheading = d(R.lheading).replace(/bull/g, R.bullet).getRegex();
      R.paragraph = d(R._paragraph)
        .replace("hr", R.hr)
        ...
        .replace("tag", R._tag)
        .getRegex();
      R.blockquote = d(R.blockquote).replace("paragraph", R.paragraph).getRegex();
      R.normal = { ...R };
      R.gfm = {
        ...R.normal,
        table:
      ...
      R.gfm.table = d(R.gfm.table)
        ...
        .replace("tag", R._tag)
        .getRegex();
      R.gfm.paragraph = d(R._paragraph)
        .replace("hr", R.hr)
        .replace("heading", " {0,3}#{1,6} ")
        .replace("|lheading", "")
      ...
      R.pedantic = {
        ...
        fences: b,
        lheading: /^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,
        paragraph: d(R.normal._paragraph)
          .replace("hr", R.hr)
          .replace("heading", " *#{1,6} *[^\n]")
      ...
      S._punctuation = "\\p{P}$+<=>`^|~";
      S.punctuation = d(S.punctuation, "u")
        .replace(/punctuation/g, S._punctuation)
        .getRegex();
      S.blockSkip = /\[[^[\]]*?\]\([^\(\)]*?\)|`[^`]*?`|<[^<>]*?>/g;
      S.anyPunctuation = /\\[punct]/g;
      S._escapes = /\\([punct])/g;
      S._comment = d(R._comment).replace("(?:--\x3e|$)", "--\x3e").getRegex();
      S.emStrong.lDelim = d(S.emStrong.lDelim, "u").replace(/punct/g, S._punctuation).getRegex();
      S.emStrong.rDelimAst = d(S.emStrong.rDelimAst, "gu").replace(/punct/g, S._punctuation).getRegex();
      S.emStrong.rDelimUnd = d(S.emStrong.rDelimUnd, "gu").replace(/punct/g, S._punctuation).getRegex();
      S.anyPunctuation = d(S.anyPunctuation, "gu").replace(/punct/g, S._punctuation).getRegex();
      S._escapes = d(S._escapes, "gu").replace(/punct/g, S._punctuation).getRegex();
      S._scheme = /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/;
      S._email =
        /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/;
      S.autolink = d(S.autolink).replace("scheme", S._scheme).replace("email", S._email).getRegex();
      S._attribute = /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/;
      S.tag = d(S.tag).replace("comment", S._comment).replace("attribute", S._attribute).getRegex();
      S._label = /(?:\[(?:\\.|[^\[\]\\])*\]|\\.|`[^`]*`|[^\[\]\\`])*?/;
      S._href = /<(?:\\.|[^\n<>\\])+>|[^\s\x00-\x1f]*/;
      S._title = /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/;
      S.link = d(S.link).replace("label", S._label).replace("href", S._href).replace("title", S._title).getRegex();
      S.reflink = d(S.reflink).replace("label", S._label).replace("ref", R._label).getRegex();
      S.nolink = d(S.nolink).replace("ref", R._label).getRegex();
      S.reflinkSearch = d(S.reflinkSearch, "g").replace("reflink", S.reflink).replace("nolink", S.nolink).getRegex();
      S.normal = { ...S };
      S.pedantic = {
        ...S.normal,
        strong: {
      ...
      S.gfm = {
        ...S.normal,
        escape: d(S.escape).replace("])", "~|])").getRegex(),
      ...
      S.gfm.url = d(S.gfm.url, "i").replace("email", S.gfm._extended_email).getRegex();
      S.breaks = {
        ...S.gfm,
        br: d(S.br).replace("{2,}", "*").getRegex(),
        text: d(S.gfm.text)
          .replace("\\b_", "\\b_| {2,}\\n")
          .replace(/\{2,\}/g, "*")
          .getRegex(),
      ...
      class T {
        ...
        constructor(e) {
          ...
          const t = { block: R.normal, inline: S.normal };
          if (this.options.pedantic) {
            t.block = R.pedantic;
            t.inline = S.pedantic;
        ...
        inlineTokens(e, t = []) {
          ...
          if (this.tokens.links) {
            ...
            if (e.length > 0) {
              while ((l = this.tokenizer.rules.inline.reflinkSearch.exec(i)) != null) {
                if (e.includes(l[0].slice(l[0].lastIndexOf("[") + 1, -1))) {
                  i = i.slice(0, l.index) + "[" + "a".repeat(l[0].length - 2) + "]" + i.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex);
                }
              }
            }
          }
          while ((l = this.tokenizer.rules.inline.blockSkip.exec(i)) != null) {
            i = i.slice(0, l.index) + "[" + "a".repeat(l[0].length - 2) + "]" + i.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);
          }
          while ((l = this.tokenizer.rules.inline.anyPunctuation.exec(i)) != null) {
          ...
          while (e) {
            ...
            if ((n = this.tokenizer.emStrong(e, i, a))) {
              e = e.substring(n.raw.length);
              t.push(n);
              continue;
            }
        ...
        parse(e, t = true) {
          ...
          for (let s = 0; s < e.length; s++) {
            ...
            switch (r.type) {
              ...
              case "heading": {
                const e = r;
                n += this.renderer.heading(this.parseInline(e.tokens), e.depth, g(this.parseInline(e.tokens, this.textRenderer)));
                continue;
              }
              case "code": {
                const e = r;
              ...
              case "table": {
                ...
                for (let n = 0; n < e.header.length; n++) {
                  s += this.renderer.tablecell(this.parseInline(e.header[n].tokens), { header: true, align: e.align[n] });
                }
                ...
                for (let n = 0; n < e.rows.length; n++) {
                  ...
                  for (let n = 0; n < t.length; n++) {
                    s += this.renderer.tablecell(this.parseInline(t[n].tokens), { header: false, align: e.align[n] });
                  }
              ...
              case "list": {
                ...
                for (let n = 0; n < e.items.length; n++) {
                  ...
                  if (t.task) {
                    ...
                    if (i) {
                      if (t.tokens.length > 0 && t.tokens[0].type === "paragraph") {
                        t.tokens[0].text = e + " " + t.tokens[0].text;
                        if (t.tokens[0].tokens && t.tokens[0].tokens.length > 0 && t.tokens[0].tokens[0].type === "text") {
              ...
              case "text": {
                let i = r;
                let l = i.tokens ? this.parseInline(i.tokens) : i.text;
      ...
      class q {
        ...
        parse = this.#e(T.lex, E.parse);
        parseInline = this.#e(T.lexInline, E.parseInline);
        Parser = E;
        parser = E.parse;
        Renderer = A;
        TextRenderer = I;
        ...
        use(...e) {
          ...
          e.forEach((e) => {
            ...
            if (e.renderer) {
              const t = this.defaults.renderer || new A(this.defaults);
            ...
            if (e.tokenizer) {
              const t = this.defaults.tokenizer || new z(this.defaults);
        ...
        #e(e, t) {
          return (n, s) => {
            ...
            try {
              if (i.hooks) {
                n = i.hooks.preprocess(n);
              }
              const s = e(n, i);
              if (i.walkTokens) {
                this.walkTokens(s, i.walkTokens);
              }
              let r = t(s, i);
              if (i.hooks) {
                r = i.hooks.postprocess(r);
              }
              return r;
            } catch (o) {
              return l(o);
        ...
        #t(e, t) {
          return (n) => {
            ...
            if (e) {
              const e = "<p>An error occurred:</p><pre>" + u(n.message + "", true) + "</pre>";
              if (t) {
      ...
      D.parse = D;
      const P = D.options;
      const v = D.setOptions;
      const B = D.use;
      const Q = D.walkTokens;
      const C = D.parseInline;
      const M = D;
      const O = E.parse;
      const j = T.lex;
    },
